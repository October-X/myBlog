...
Mock.mock(/^\/api\/blog(\?.+)?$/, "get", function (options) {
  //返回的是函数的执行结果
  const query = qs.parse(options.url); //讲参数中的url格式化
...
Mock.mock(/^\/api\/blog\/[^/]+$/, "get", {//最后部分表示只要非斜杆的数据就可以
  code: 0,
  msg: "",
  data: {
    id: "@guid",
...
Mock.mock(/^\/api\/comment\/?(\?.+)?$/, "get", function (options) {//可匹配comment/,/零次或一次 或者comment?asdfasdf
